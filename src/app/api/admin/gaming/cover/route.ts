...
export async function POST(req: NextRequest) {
  try {
    ...
    const mood = (note || article || title).slice(0, 400);
    const prompt = `Cinematic 16:9 editorial game-culture photograph for an article titled "${title}". Mood from the piece: ${mood}. Photoreal, moody, dark den lighting, no logos, no readable text, no UI, fill the frame edge to edge.`;
    ...
    return NextResponse.json({ cover: pub.publicUrl });
  } catch (err: any) {
